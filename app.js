...
(function() {
  ...
  const storedMonth = localStorage.getItem('mealAppMonth');

  // Reset if month changed
  if (storedMonth && parseInt(storedMonth) !== currentMonth) {
    localStorage.clear();
    localStorage.setItem('mealAppMonth', currentMonth);
    alert("New month detected! All previous data has been reset.");
  } else if (!storedMonth) {
    localStorage.setItem('mealAppMonth', currentMonth);
  }

  // Save all input data automatically
  ...
  // Save and restore table body
  const tableBody = document.querySelector('#mealTable tbody');

  // Restore table data
  const savedTable = localStorage.getItem('mealTableData');
  if (savedTable) {
  ...
  const observer = new MutationObserver(() => {
    localStorage.setItem('mealTableData', tableBody.innerHTML);
  });
  observer.observe(tableBody, { childList: true, subtree: true, characterData: true });

  // Optional: manual reset button (you can remove if not needed)
